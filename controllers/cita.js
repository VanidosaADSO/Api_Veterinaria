...
const postCita = async (req, res) => {
    const { Documento, Servicios, Mascota, Veterinario, Fecha_Hora, Disponibilidad, Cupon, Estado } = req.body

    const cita1 = new cita({ Documento, Servicios, Mascota, Veterinario, Fecha_Hora, Disponibilidad, Cupon, Estado })
...
const putcita = async (req, res) => {
    ...
    const { _id, Documento, Servicios, Mascota, Veterinario, Fecha_Hora, Disponibilidad, Cupon, Estado } = req.body;
    ...
    if (Servicios && Array.isArray(Servicios)) {
        try {
            ...
            const updatedServicio = await cita.findByIdAndUpdate(
                { _id: _id },
                { Servicios: updatedServicios, Mascota, Veterinario, Documento, Fecha_Hora, Disponibilidad, Cupon, Estado },
                { new: true }
            );
